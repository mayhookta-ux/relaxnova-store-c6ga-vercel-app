...
export default function App() {
  ...
  const footer = <footer className="footer"><div className="footer-brand"><strong>{businessInfo.storeName}</strong><span>Premium single-product posture support for US buyers.</span><small>© RelaxNova. All rights reserved. Product content is focused on posture-awareness support and does not make medical claims.</small></div><nav className="footer-legal" aria-label="Legal footer"><strong>Store policies</strong>{legalLinks.map((link) => <a key={link.key} href={`#${link.key}`}>{link.label}</a>)}</nav><div className="footer-trust"><strong>Buyer protection</strong><span><Truck size={15} /> Free US Shipping</span><span><RotateCcw size={15} /> 30-day return review</span><span><ShieldCheck size={15} /> Secure Stripe checkout</span><span><Mail size={15} /> {businessInfo.email}</span></div><div className="footer-support"><strong>RelaxNova support</strong><span><Mail size={15} /> {businessInfo.email}</span><span><MapPin size={15} /> {businessInfo.location}</span><span><Clock3 size={15} /> 1–3 business day response</span><span><PackageCheck size={15} /> US delivery estimate: 8–23 days after processing</span></div></footer>;
