...
const supportEmail = "relaxnova.store.support@gmail.com";
const fallbackAnswer = `I don’t want to guess on that. Please visit our Contact Us page or email ${supportEmail}, and RelaxNova support will help within 1–3 business days.`;
const storeContext = `Store context only: RelaxNova sells the Smart Posture Corrector, a black adjustable upper-back posture trainer with gentle vibration reminders, LED reminder counter, adjustable strap, daily posture-awareness support. Support email: ${supportEmail}. It supports better posture habits and comfort awareness, but is not medical treatment. Price offer is $39 with Free US Shipping included. US processing is usually 1–3 business days, sometimes up to 5 during delays. US delivery is typically 8–23 calendar days after processing. Orders may be fulfilled through logistics partners, so tracking can take 3–7 days to update after label/logistics creation. Checkout is securely processed by Stripe; the storefront does not store full card numbers. Eligible return requests may be reviewed within 30 days after confirmed delivery. Returns require written approval first and items should be complete, clean, undamaged, safely packed, and include parts/accessories. Damaged, defective, or wrong item claims should be reported within 7 days with photos/video. Carrier delays, wrong addresses, refused packages, heavy use, missing parts, misuse, or policy abuse may not qualify for refund.`;
...
function localAnswer(question: string) {
  ...
  if (/ship|delivery|tracking|arrive|carrier|fulfill/.test(q)) return "Most US orders process in 1–3 business days, then delivery is typically 8–23 days. Carrier handoffs can make tracking take a few days to update.";
  if (/refund|return|exchange|money back|cancel/.test(q)) return `Eligible orders can be reviewed within 30 days after delivery. Email RelaxNova at ${supportEmail} first—items should be complete, clean, undamaged, and approved before return shipping.`;
  ...
  if (/order|email|support|wrong|damaged|defective|where/.test(q)) return `For order help, email ${supportEmail} with your order number, checkout email, shipping ZIP code, and photos if there’s a product or package issue.`;
  return "";
}
  ...
  try {
    ...
    const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: `You are a real human-style premium US ecommerce support assistant. Use ONLY this store context and do not invent facts: ${storeContext}\nReply warmly, naturally, professionally, and under 45 words. Be conversion-friendly and trust-building. Answer only product usage, posture benefits, shipping, free shipping, refunds, Stripe payment safety, order process, tracking, delivery timing, and checkout trust questions. For anything outside this context, politely direct the customer to the Contact Us page. Never make medical guarantees.` },
          ...cleanMessages,
    ...
    return new Response(JSON.stringify({ answer: fallbackAnswer }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});